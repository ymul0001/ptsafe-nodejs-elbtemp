...
const findDistinctRoutes = async (req,res) => {
    ...
    if (!StringUtils.isNullOrEmpty(stopId) && !StringUtils.isNullOrEmpty(directionType) && !StringUtils.isNullOrEmpty(lat) && !StringUtils.isNullOrEmpty(long)) {
        const routes = await Report.listDistinctRoutesForFilter(parseInt(stopId), parseInt(directionType), parseFloat(lat), parseFloat(long));
